...
void Madd(my256d *&Csr2_mid_val,my256d *&Csr2_mtx_val,INT *&col_idx,double *&vec_val,INT *& par_set){
    ...
    my256d* Csr2_mid_val1_t=new my256d[i_end];
    my256d* Csr2_col_val_t=new my256d[i_end];
    my256d* tmp_t=new my256d[i_end];
    my256dset(Csr2_mid_val1_t,i_end);
    my256dset(Csr2_col_val_t,i_end);
    my256dset(tmp_t,i_end);
    auto func = [&](int start, int end){
        for(int i = start;i<end;i++){
            INT   xx,yy,zz;
            my256d* Csr2_mid_val1=&Csr2_mid_val1_t[i];
            my256d* Csr2_col_val= &Csr2_col_val_t[i];
            my256d* tmp_t= &tmp_t[i];
    ...
    auto thread0_ = std::thread(func, 0, i_end/4);
    auto thread1_ = std::thread(func, i_end/4, i_end/2);
    auto thread2_ = std::thread(func, i_end/2, i_end*3/4);
    auto thread3_ = std::thread(func, i_end*3/4, i_end);
    pthread_t handle0 = thread0_.native_handle();
    pthread_t handle1 = thread1_.native_handle();
    pthread_t handle2 = thread2_.native_handle();
    pthread_t handle3 = thread3_.native_handle();
    cpu_set_t cpuset0;
    cpu_set_t cpuset1;
    cpu_set_t cpuset2;
    cpu_set_t cpuset3;
    ...
    CPU_SET(2, &cpuset2);
    CPU_SET(3, &cpuset3);
    int result0 = pthread_setaffinity_np(handle0, sizeof(cpu_set_t), &cpuset0);
    int result1 = pthread_setaffinity_np(handle1, sizeof(cpu_set_t), &cpuset1);
    int result2 = pthread_setaffinity_np(handle2, sizeof(cpu_set_t), &cpuset2);
    int result3 = pthread_setaffinity_np(handle3, sizeof(cpu_set_t), &cpuset3);
    thread0_.join();
    thread1_.join();
    thread2_.join();
    thread3_.join();
...
void SegSum(INT *&Csr2_row_ptr,my256d *&Csr2_mid_val,double *&mtx_ans,INT *&par_set){
    ...
    auto func = [&](int start, int end){
        ...
        for(;start<end;start++){
            double s=0;
            INT t1,t2;
            t1=Csr2_row_ptr[start];
            t2=Csr2_row_ptr[start+1];
            for(j=t1;j<t2;j++)
            {
                INT t=j>>2;
                INT p = j-(t<<2);
    ...
    auto thread0_ = std::thread(func, 0, num/4);
    auto thread1_ = std::thread(func, num/4, num/2);
    auto thread2_ = std::thread(func, num/2, num*3/4);
    auto thread3_ = std::thread(func, num*3/4, num);
    pthread_t handle0 = thread0_.native_handle();
    pthread_t handle1 = thread1_.native_handle();
    pthread_t handle2 = thread2_.native_handle();
    pthread_t handle3 = thread3_.native_handle();
...
int main(int argc,char ** argv){
    ...
    my256d  * Csr2_mtx_val;
    my256d  * Csr2_mid_val;
    fp_ans = fopen("answer_CSR2_S_MP.mtx","wb+");
    char * Matrixname;
    char * Vectorname;
    char * Iterations; 
    if(argc>3)
